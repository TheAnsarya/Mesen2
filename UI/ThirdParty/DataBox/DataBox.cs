...
public class DataBox : TemplatedControl {
	...
	public ISelectionModel Selection {
		get;
		set => SetAndRaise(SelectionProperty, ref field, value);
	} = new SelectionModel<object?>();
	...
	internal void OnCellDoubleTapped(object? sender, RoutedEventArgs e) {
		if (sender is DataBoxCell cell && cell.Column is not DataBoxCheckBoxColumn) {
			Dispatcher.UIThread.Post(() => CellDoubleClick?.Invoke(cell));
		}
	}
	...
	internal void OnCellPointerReleased(object? sender, PointerReleasedEventArgs e) {
		if (sender is DataBoxCell cell && _pressedCell == cell) {
			Dispatcher.UIThread.Post(() => CellClick?.Invoke(cell));
		}
...
public class SortState {
	...
	public void ToggleSortOrder(string column, bool reset) {
		...
		ListSortDirection order = i < 0
			...
			: SortOrder[i].Item2 == ListSortDirection.Descending ? ListSortDirection.Ascending : ListSortDirection.Descending;
