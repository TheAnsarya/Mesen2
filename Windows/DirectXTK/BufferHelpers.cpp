...
_Use_decl_annotations_
	...
	if (SUCCEEDED(hr)) {
		...
		if (desc.MipLevels != 1) {
			...
			while (device->IsFencePending(copyFence)) {
				SwitchToThread();
			}
#else
...
_Use_decl_annotations_
	...
	if (!device || !width || !height || !depth || !initData.pSysMem || !initData.SysMemPitch || !initData.SysMemSlicePitch)
		return E_INVALIDARG;
	...
	if ((width > D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION) || (height > D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION) || (depth > D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION)) {
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ void Internal::ConstantBufferBase::CreateBuffer(
    ID3D11Device* device,
    size_t bytes,
