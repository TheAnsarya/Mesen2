...
void WsDisUtils::GetDisassembly(DisassemblyInfo& info, string& out, uint32_t memoryAddr, LabelManager* labelManager, EmuSettings* settings) {
	...
	while (IsPrefix(opCode)) {
		switch (opCode) {
			case 0x26:
				segment = WsSegment::ES;
				break;
			case 0x36:
				segment = WsSegment::SS;
				break;
			case 0x2E:
				segment = WsSegment::CS;
				break;
			case 0x3E:
				segment = WsSegment::DS;
				break;
	...
	while (op[i]) {
		switch (op[i]) {
			...
			// Relative jumps
			case 'r':
				GetJmpDestination(str, byteCode + 1, 1);
				break;
			case 's':
				GetJmpDestination(str, byteCode + 1, 2);
				break;
			case 't':
				GetJmpDestination(str, byteCode + 1, 4);
				break;
			...
			case 'x': { // GRP3
				...
				switch (mode) {
					...
					case 3:
						...
						break;
					case 4:
						str.WriteAll("MUL ", word ? "AX, " : "");
						break;
					case 5:
						str.WriteAll("IMUL ", word ? "AX, " : "");
						break;
					case 6:
						str.WriteAll("DIV ", word ? "AX, " : "");
						break;
					case 7:
						str.WriteAll("IDIV ", word ? "AX, " : "");
						break;
				}
				byteCode += GetModRmParam(str, byteCode, segment, word);
				if (mode == 0) {
...
void WsDisUtils::GetSegment(FastString& str, WsSegment segment, const char* defaultSegment) {
	switch (segment) {
		default:
		case WsSegment::Default:
			str.WriteAll(defaultSegment, ':');
			break;
		case WsSegment::ES:
			str.Write("ES:");
			break;
