...
void WsDebugger::Step(int32_t stepCount, StepType type) {
	...
	switch (type) {
		...
		case StepType::PpuScanline:
			step.PpuStepCount = 256 * stepCount;
			break;
		case StepType::PpuFrame:
			step.PpuStepCount = 256 * _ppu->GetScanlineCount() * stepCount;
			break;
...
bool WsDebugger::SaveRomToDisk(const string& filename, bool saveAsIps, CdlStripOption stripOption) {
