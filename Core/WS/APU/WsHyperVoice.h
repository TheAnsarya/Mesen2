...
public:
	...
	int16_t ConvertSampleToOutput(uint8_t sample) {
		...
		switch (_state->ScalingMode) {
			case WsHyperVoiceScalingMode::Unsigned:
				output = sample << 8;
				break;
			case WsHyperVoiceScalingMode::UnsignedNegated:
				output = 0xFFFF0000 | (sample << 8);
				break;
			case WsHyperVoiceScalingMode::Signed:
				output = ((int32_t)(int8_t)sample) << 8;
				break;
			case WsHyperVoiceScalingMode::None:
				output = (int16_t)(sample << 8);
		}
	...
	uint8_t Read(uint16_t port) {
		switch (port) {
			case 0x6A:
				return _state->ControlLow;
			case 0x6B:
				return _state->ControlHigh;
		}
	...
	void Write(uint16_t port, uint8_t value) {
		switch (port) {
			case 0x64:
				BitUtilities::SetBits<0>(_state->LeftOutput, value);
				break;
			case 0x65:
				BitUtilities::SetBits<8>(_state->LeftOutput, value);
				break;
			case 0x66:
				BitUtilities::SetBits<0>(_state->RightOutput, value);
				break;
			case 0x67:
				BitUtilities::SetBits<8>(_state->RightOutput, value);
				break;

			case 0x69:
				WriteStereoInput(value);
				break;
