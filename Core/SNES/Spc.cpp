...
uint8_t Spc::DebugRead(uint16_t addr) {
	...
	switch (addr) {
		...
		case 0xF2:
			return _state.DspReg;
		case 0xF3:
			return _dsp->Read(_state.DspReg & 0x7F);

		case 0xF4:
			return _state.CpuRegs[0];
		case 0xF5:
			return _state.CpuRegs[1];
		case 0xF6:
			return _state.CpuRegs[2];
		case 0xF7:
			return _state.CpuRegs[3];

		case 0xF8:
			return _state.RamReg[0];
		case 0xF9:
			return _state.RamReg[1];
		...
		case 0xFD:
			return _state.Timer0.DebugRead();
		case 0xFE:
			return _state.Timer1.DebugRead();
		case 0xFF:
			return _state.Timer2.DebugRead();
...
uint8_t Spc::Read(uint16_t addr, MemoryOperationType type) {
	...
	if (addr >= 0xFFC0 && _state.RomEnabled) {
		...
		switch (addr) {
			...
			case 0xF4:
				value = _state.CpuRegs[0];
				break;
			case 0xF5:
				value = _state.CpuRegs[1];
				break;
			case 0xF6:
				value = _state.CpuRegs[2];
				break;
			case 0xF7:
				value = _state.CpuRegs[3];
				break;

			case 0xF8:
				value = _state.RamReg[0];
				break;
			case 0xF9:
				value = _state.RamReg[1];
				break;
			...
			case 0xFD:
				value = _state.Timer0.GetOutput();
				break;
			case 0xFE:
				value = _state.Timer1.GetOutput();
				break;
			case 0xFF:
				value = _state.Timer2.GetOutput();
				break;
...
void Spc::Write(uint16_t addr, uint8_t value, MemoryOperationType type) {
	...
	switch (addr) {
		...
		case 0xF4:
			_state.OutputReg[0] = value;
			break;
		case 0xF5:
			_state.OutputReg[1] = value;
			break;
		case 0xF6:
			_state.OutputReg[2] = value;
			break;
		case 0xF7:
			_state.OutputReg[3] = value;
			break;
		case 0xF8:
			_state.RamReg[0] = value;
			break;
		case 0xF9:
			_state.RamReg[1] = value;
			break;

		case 0xFA:
			_state.Timer0.SetTarget(value);
			break;
		case 0xFB:
			_state.Timer1.SetTarget(value);
			break;
		case 0xFC:
			_state.Timer2.SetTarget(value);
			break;
