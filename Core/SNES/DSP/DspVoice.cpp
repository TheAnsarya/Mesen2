		...
		if (_shared->Adsr1 & 0x80) {
			...
			if (gain & 0x80) {
				...
				switch (gain & 0x60) {
					...
					case 0x60:
						env += ((uint16_t)_prevCalculatedEnv < 0x600 ? 32 : 8);
						break;
				}
			} else {
...
void DspVoice::Step3c() {
	...
	switch (_cfg->InterpolationType) {
		case DspInterpolationType::Gauss:
			output = DspInterpolation::Gauss(_interpolationPos, _sampleBuffer, _bufferPos);
			break;
		case DspInterpolationType::Cubic:
			output = DspInterpolation::Cubic(_interpolationPos, _sampleBuffer, _bufferPos);
			break;
		case DspInterpolationType::Sinc:
			output = DspInterpolation::Sinc(_interpolationPos, _sampleBuffer, _bufferPos);
			break;
		case DspInterpolationType::None:
			output = _sampleBuffer[((_interpolationPos >> 12) + _bufferPos) % 12];
			break;
	}
