...
uint8_t Sdd1::Read(uint32_t addr) {
	if ((uint16_t)addr >= 0x4800 && (uint16_t)addr <= 0x4807) {
		switch (addr & 0x07) {
			case 0:
				return _state.AllowDmaProcessing;
			case 1:
				return _state.ProcessNextDma;
...
void Sdd1::Write(uint32_t addr, uint8_t value) {
	if ((uint16_t)addr >= 0x4800 && (uint16_t)addr <= 0x4807) {
		...
		switch (addr & 0x07) {
			case 0:
				_state.AllowDmaProcessing = value;
				break;
			case 1:
				_state.ProcessNextDma = value;
				break;
		...
		if ((uint16_t)addr >= 0x4300 && (uint16_t)addr <= 0x437A) {
			...
			switch (addr & 0x0F) {
				case 0x02:
					_state.DmaAddress[ch] = (_state.DmaAddress[ch] & 0xFFFF00) | value;
					break;
				case 0x03:
					_state.DmaAddress[ch] = (_state.DmaAddress[ch] & 0xFF00FF) | (value << 8);
					break;
				case 0x04:
					_state.DmaAddress[ch] = (_state.DmaAddress[ch] & 0x00FFFF) | (value << 16);
					break;
				case 0x05:
					_state.DmaLength[ch] = (_state.DmaLength[ch] & 0xFF00) | value;
					break;
				case 0x06:
					_state.DmaLength[ch] = (_state.DmaLength[ch] & 0x00FF) | (value << 8);
					break;
			}
		}
