...
uint8_t Obc1::Read(uint32_t addr) {
	...
	switch (addr) {
		case 0x1FF0:
			return ReadRam(GetLowAddress());
		case 0x1FF1:
			return ReadRam(GetLowAddress() + 1);
		case 0x1FF2:
			return ReadRam(GetLowAddress() + 2);
		case 0x1FF3:
			return ReadRam(GetLowAddress() + 3);
		case 0x1FF4:
			return ReadRam(GetHighAddress());
	}
	return ReadRam(addr);
}
...
void Obc1::Write(uint32_t addr, uint8_t value) {
	...
	switch (addr) {
		case 0x1FF0:
			WriteRam(GetLowAddress(), value);
			break;
		case 0x1FF1:
			WriteRam(GetLowAddress() + 1, value);
			break;
		case 0x1FF2:
			WriteRam(GetLowAddress() + 2, value);
			break;
		case 0x1FF3:
			WriteRam(GetLowAddress() + 3, value);
			break;
		case 0x1FF4: {
