...
uint8_t InternalRegisters::Peek(uint16_t addr) {
	...
	switch (addr) {
		case 0x4210:
			return (_nmiFlag ? 0x80 : 0) | 0x02 | (_memoryManager->GetOpenBus() & 0x70);
		case 0x4211:
			return (_irqFlag ? 0x80 : 0) | (_memoryManager->GetOpenBus() & 0x7F);
		...
		case 0x421A:
			return (uint8_t)(_state.ControllerData[1] >> 8);
		case 0x421B:
			return (uint8_t)(_state.ControllerData[1] >> 8);
		case 0x421C:
			return (uint8_t)(_state.ControllerData[2] >> 8);
		case 0x421D:
			return (uint8_t)(_state.ControllerData[2] >> 8);
		case 0x421E:
			return (uint8_t)(_state.ControllerData[3] >> 8);
		case 0x421F:
			return (uint8_t)(_state.ControllerData[3] >> 8);
...
uint8_t InternalRegisters::Read(uint16_t addr) {
	switch (addr) {
		...
		case 0x4218:
			return ReadControllerData(0, false);
		case 0x4219:
			return ReadControllerData(0, true);
		case 0x421A:
			return ReadControllerData(1, false);
		case 0x421B:
			return ReadControllerData(1, true);
		case 0x421C:
			return ReadControllerData(2, false);
		case 0x421D:
			return ReadControllerData(2, true);
		case 0x421E:
			return ReadControllerData(3, false);
		case 0x421F:
			return ReadControllerData(3, true);
...
void InternalRegisters::Write(uint16_t addr, uint8_t value) {
	switch (addr) {
		...
		case 0x420D:
			_state.EnableFastRom = (value & 0x01) != 0;
			break;
