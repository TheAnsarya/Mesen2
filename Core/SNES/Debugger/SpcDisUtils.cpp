...
void SpcDisUtils::GetDisassembly(DisassemblyInfo& info, string& out, uint32_t memoryAddr, LabelManager* labelManager, EmuSettings* settings) {
	...
	while (op[i]) {
		switch (op[i]) {
			case 'r':
				getOperand((uint16_t)(memoryAddr + (int8_t)byteCode[1] + GetOpSize(byteCode[0])));
				break;
			case 'q':
				getOperand((uint16_t)(memoryAddr + (int8_t)byteCode[2] + GetOpSize(byteCode[0])));
				break; // relative 2nd byte

			case 'a':
				getOperand((uint16_t)(byteCode[1] | (byteCode[2] << 8)));
				break;
			...
			case 'd':
				...
				break;
			case 'e':
				str.WriteAll('$', HexUtilities::ToHex(byteCode[2]));
				break; // direct 2nd byte
			...
			case 'm':
				getOperand((uint16_t)((byteCode[1] | (byteCode[2] << 8)) & 0x1FFF));
				break;
			case 'b':
				str.WriteAll((char)('0' + (byteCode[2] >> 5)));
				break;
