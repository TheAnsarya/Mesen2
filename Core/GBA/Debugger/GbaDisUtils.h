...
private:
	...
	[[nodiscard]] static bool IsArmBranch(uint32_t opCode);
...
public:
	...
	[[nodiscard]] static bool IsJumpToSub(uint32_t opCode, uint8_t flags);
	[[nodiscard]] static bool IsReturnInstruction(uint32_t opCode, uint8_t flags);
	[[nodiscard]] static bool IsUnconditionalJump(uint32_t opCode, uint8_t flags);
	[[nodiscard]] static bool IsConditionalJump(uint32_t opCode, uint8_t flags);
	static CdlFlags::CdlFlags GetOpFlags(uint32_t opCode, uint8_t flags, uint32_t pc, uint32_t prevPc);
	[[nodiscard]] static bool IsThumbMode(uint8_t flags);
};
