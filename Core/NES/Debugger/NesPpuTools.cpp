...
DebugTilemapInfo NesPpuTools::GetTilemap(GetTilemapOptions options, BaseState& baseState, BaseState& ppuToolsState, uint8_t* vram, uint32_t* palette, uint32_t* outBuffer) {
	...
	switch (_mapper->GetState().Mirroring) {
		case MirroringType::Horizontal:
			result.Mirroring = TilemapMirroring::Horizontal;
			break;
		case MirroringType::Vertical:
			result.Mirroring = TilemapMirroring::Vertical;
			break;
		case MirroringType::ScreenAOnly:
			result.Mirroring = TilemapMirroring::SingleScreenA;
			break;
		case MirroringType::ScreenBOnly:
			result.Mirroring = TilemapMirroring::SingleScreenB;
			break;
		case MirroringType::FourScreens:
			result.Mirroring = TilemapMirroring::FourScreens;
			break;
	}
