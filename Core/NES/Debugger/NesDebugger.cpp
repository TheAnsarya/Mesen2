...
NesDebugger::NesDebugger(Debugger* debugger) : IDebugger(debugger->GetEmulator()) {
	...
	switch (_console->GetRomFormat()) {
		case RomFormat::Fds:
			cdlFile = "FdsFirmware.cdl";
			break;
		case RomFormat::StudyBox:
			cdlFile = "StudyBoxFirmware.cdl";
			break;
		default:
			cdlFile = _emu->GetRomInfo().RomFile.GetFileName();
			break;
	}
...
void NesDebugger::Step(int32_t stepCount, StepType type) {
	...
	switch (type) {
		...
		case StepType::PpuFrame:
			step.PpuStepCount = 341 * _console->GetPpu()->GetScanlineCount() * stepCount;
			break;
