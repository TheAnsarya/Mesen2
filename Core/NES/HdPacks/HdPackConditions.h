...
struct HdPackBaseMemoryCondition : public HdPackCondition {
	...
	string ToString() override {
		...
		switch (Operator) {
			case HdPackConditionOperator::Equal:
				out << "==";
				break;
			case HdPackConditionOperator::NotEqual:
				out << "!=";
				break;
			case HdPackConditionOperator::GreaterThan:
				out << ">";
				break;
			case HdPackConditionOperator::LowerThan:
				out << "<";
				break;
			case HdPackConditionOperator::LowerThanOrEqual:
				out << "<=";
				break;
			case HdPackConditionOperator::GreaterThanOrEqual:
				out << ">=";
				break;
		}
		out << ",";
	...
	bool InternalCheckCondition(int x, int y, HdPpuTileInfo* tile) override {
		...
		switch (Type) {
			...
			case HdPackConditionType::PositionCheckY:
				...
				break;
			case HdPackConditionType::OriginPositionCheckX:
				val = tile->HorizontalMirroring ? (uint32_t)(x - (7 - tile->OffsetX)) : (uint32_t)(x - tile->OffsetX);
				break;
			case HdPackConditionType::OriginPositionCheckY:
				val = tile->VerticalMirroring ? (uint32_t)(y - (7 - tile->OffsetY)) : (uint32_t)(y - tile->OffsetY);
				break;
		}
