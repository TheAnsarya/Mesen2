...
void Rainbow::SelectLowBank(uint16_t start, uint16_t size, uint8_t reg) {
	switch ((_lowBanks[reg] & 0xC000) >> 14) {
		...
		case 1:
			...
			break;
		case 2:
			SetCpuMemoryMapping(start, start + size - 1, GetWorkRamType(), (_lowBanks[reg] & 0x3FFF) * size, MemoryAccessType::ReadWrite);
			break;
		case 3:
			SetCpuMemoryMapping(start, start + size - 1, PrgMemoryType::MapperRam, (_lowBanks[reg] & 0x3FFF) * size, MemoryAccessType::ReadWrite);
			break;
	}
}
...
void Rainbow::UpdateState() {
	...
	for (int i = 0; i < 4; i++) {
		...
		switch (ctrl.Source) {
			case 0:
				SetPpuMemoryMapping(start, end, ChrMemoryType::NametableRam, _ntBanks[i] * 0x400, MemoryAccessType::ReadWrite);
				break;
			case 1:
				SetPpuMemoryMapping(start, end, ChrMemoryType::ChrRam, _ntBanks[i] * 0x400, MemoryAccessType::ReadWrite);
				break;
			case 2:
				SetPpuMemoryMapping(start, end, ChrMemoryType::MapperRam, (_ntBanks[i] & 0x03) * 0x400, MemoryAccessType::ReadWrite);
				break;
			case 3:
				SetPpuMemoryMapping(start, end, ChrMemoryType::ChrRom, _ntBanks[i] * 0x400, MemoryAccessType::Read);
				break;
		}
	}
}
...
uint8_t Rainbow::ReadChr(uint32_t addr) {
	switch (_chrSource) {
		default:
		case 0:
			return _chrRomSize ? _chrRom[addr & (_chrRomSize - 1)] : 0;
		case 1:
			return _chrRamSize ? _chrRam[addr & (_chrRamSize - 1)] : 0;
...
uint8_t Rainbow::ReadRegister(uint16_t addr) {
	switch (addr) {
		case 0x4100:
			return _highMode | (_lowMode << 7);
		case 0x4120:
			return _chrMode | ((uint8_t)_windowEnabled << 4) | ((uint8_t)_spriteExtMode << 5) | (_chrSource << 6);
		...
		case 0x4190:
			return (uint8_t)_espEnabled | ((uint8_t)_wifiIrqEnabled << 1);
		case 0x4191:
			return ((uint8_t)_dataReady << 6) | ((uint8_t)_dataReceived << 7);
		case 0x4192:
			return (uint8_t)_dataSent << 7;
...
void Rainbow::WriteRegister(uint16_t addr, uint8_t value) {
	switch (addr) {
		...
		case 0x4121:
			_bgExtModeOffset = value & 0x1F;
			break;

		case 0x4124:
			_fillModeTileIndex = value;
			break;
		case 0x4125:
			_fillModeAttrIndex = value & 0x03;
			break;
		case 0x412E:
			_windowBank = value;
			break;
		...
		case 0x4153:
			_slIrqOffset = std::clamp<uint8_t>(value, 1, 170);
			break;

		case 0x4158:
			BitUtilities::SetBits<8>(_cpuIrqReloadValue, value);
			break;
		case 0x4159:
			BitUtilities::SetBits<0>(_cpuIrqReloadValue, value);
			break;
		case 0x415A:
		...
		case 0x415C:
			BitUtilities::SetBits<8>(_fpgaRamAddr, value & 0x1F);
			break;
		case 0x415D:
			BitUtilities::SetBits<0>(_fpgaRamAddr, value);
			break;
		case 0x415E:
			_fpgaRamInc = value;
			break;
		...
		case 0x416C:
			BitUtilities::SetBits<8>(_nmiVectorAddr, value);
			break;
		case 0x416D:
			BitUtilities::SetBits<0>(_nmiVectorAddr, value);
			break;
		case 0x416E:
			BitUtilities::SetBits<8>(_irqVectorAddr, value);
			break;
		case 0x416F:
			BitUtilities::SetBits<0>(_irqVectorAddr, value);
			break;

		case 0x4170:
			_windowX1 = value & 0x1F;
			break;
		case 0x4171:
			_windowX2 = value & 0x1F;
			break;
		case 0x4172:
			_windowY1 = value;
			break;
		case 0x4173:
			...
			break;
		case 0x4174:
			_windowScrollX = value & 0x1F;
			break;
		case 0x4175:
			_windowScrollY = value;
			break;
		...
		case 0x4192:
			...
			break;
		case 0x4193:
			_recvDstAddr = value & 0x07;
			break;
		case 0x4194:
			_sendSrcAddr = value & 0x07;
			break;

		case 0x4240:
			_spriteExtBank = value & 0x07;
			break;
		case 0x4241:
			_oamSlowUpdatePage = value & 0x07;
			break;
		case 0x4242:
			_oamExtUpdatePage = value & 0x1F;
			break;
	}
