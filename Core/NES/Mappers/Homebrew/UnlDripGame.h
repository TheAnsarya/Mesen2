...
protected:
	...
	uint8_t ReadRegister(uint16_t addr) override {
		switch (addr & 0x5800) {
			case 0x4800:
				return (GetDipSwitches() ? 0x80 : 0) | 0x64;
			case 0x5000:
				return _audioChannels[0]->ReadRegister();
			case 0x5800:
				return _audioChannels[1]->ReadRegister();
		}
		return 0;
	}
		...
		if (addr <= 0xBFFF) {
			switch (addr & 0x800F) {
				...
				case 0x800D:
					...
					break;
				case 0x800E:
					SelectChrPage(2, value & 0x0F);
					break;
				case 0x800F:
					SelectChrPage(3, value & 0x0F);
					break;
			}
		} else {
