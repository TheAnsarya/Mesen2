...
DebugTilemapInfo SmsVdpTools::GetTilemap(GetTilemapOptions options, BaseState& baseState, BaseState& ppuToolsState, uint8_t* vram, uint32_t* palette, uint32_t* outBuffer) {
	...
	if (options.Layer == 1) {
		...
		for (int i = 0, len = (int)(result.RowCount * result.ColumnCount); i < len; i += 2) {
			switch (_memAccess[i]) {
				case SmsVdpMemAccess::None:
					outBuffer[i] = 0xFF606060;
					break;
				case SmsVdpMemAccess::BgLoadTable:
					outBuffer[i] = 0xFF00FF00;
					break;
				case SmsVdpMemAccess::BgLoadTile:
					outBuffer[i] = 0xFF0000FF;
					break;
				case SmsVdpMemAccess::SpriteEval:
					outBuffer[i] = 0xFFFF0000;
					break;
				case SmsVdpMemAccess::SpriteLoadTable:
					outBuffer[i] = 0xFF00E080;
					break;
				case SmsVdpMemAccess::SpriteLoadTile:
					outBuffer[i] = 0xFF0080E0;
					break;
				case SmsVdpMemAccess::CpuSlot:
					outBuffer[i] = 0xFFA0A0A0;
					break;
			}
