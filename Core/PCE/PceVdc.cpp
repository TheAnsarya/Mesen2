		...
		for (uint16_t i = _loadBgLastCycle; i < end; i++) {
			if constexpr (skipRender) {
				...
				switch (i & 0x07) {
					...
					case 1:
						LoadBatEntry(scrollOffset, columnMask, row);
						break;
					case 3:
						_allowVramAccess = false;
						break; // Unused BAT read?
					case 5:
						LoadTileDataCg0(row);
						break;
					case 7:
...
uint8_t PceVdc::ReadRegister(uint16_t addr) {
	...
	switch (addr & 0x03) {
		...
		case 1:
			return 0; // Unused, reads return 0
...
void PceVdc::WriteRegister(uint16_t addr, uint8_t value) {
	...
	switch (addr & 0x03) {
		case 0:
			_state.CurrentReg = value & 0x1F;
			break;

		case 1:
			break; // Unused, writes do nothing
		...
		case 3:
			...
			switch (_state.CurrentReg) {
					...
					if (msb) {
						...
						switch ((value >> 3) & 0x03) {
							case 0:
								...
								break;
							case 1:
								_state.VramAddrIncrement = 0x20;
								break;
							case 2:
								_state.VramAddrIncrement = 0x40;
								break;
							case 3:
								_state.VramAddrIncrement = 0x80;
								break;
						}
					} else {
				...
				case 0x06:
					UpdateReg<0x3FF>(_state.RasterCompareRegister, value, msb);
					break;
