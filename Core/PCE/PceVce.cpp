...
uint8_t PceVce::Read(uint16_t addr) {
	switch (addr & 0x07) {
		default:
		case 0:
			return 0xFF; // write-only, reads return $FF
		case 1:
			return 0xFF; // unused, reads return $FF
		case 2:
			return 0xFF; // write-only, reads return $FF
		case 3:
			return 0xFF; // write-only, reads return $FF
		...
		case 4:
			return _paletteRam[_state.PalAddr] & 0xFF;
...
void PceVce::Write(uint16_t addr, uint8_t value) {
	switch (addr & 0x07) {
		...
		case 0x02:
			_state.PalAddr = (_state.PalAddr & 0x100) | value;
			break;
		case 0x03:
			_state.PalAddr = (_state.PalAddr & 0xFF) | ((value & 0x01) << 8);
			break;
