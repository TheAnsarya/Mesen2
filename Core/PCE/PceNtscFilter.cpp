...
void PceNtscFilter::ApplyFilter(uint16_t* ppuOutputBuffer) {
	...
	uint32_t frameWidth = _frameDivider ? (PceConstants::GetRowWidth(_frameDivider) - (overscan.Left + overscan.Right) * 4 / _frameDivider) : (_baseFrameInfo.Width - (overscan.Left + overscan.Right) * PceConstants::InternalResMultipler);
