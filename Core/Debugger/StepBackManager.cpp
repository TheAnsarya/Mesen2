...
void StepBackManager::StepBack(StepBackType type) {
	if (!_active) {
		...
		switch (type) {
			default:
			case StepBackType::Instruction:
				target = cfg.CurrentCycle;
				break;
			case StepBackType::Scanline:
				target = (int64_t)cfg.CurrentCycle - cfg.CyclesPerScanline;
				break;
			case StepBackType::Frame:
				target = (int64_t)cfg.CurrentCycle - cfg.CyclesPerFrame;
				break;
		}
...
bool StepBackManager::CheckStepBack() {
	...
	if (clock < _targetClock && _targetClock - clock < _stateClockLimit) {
		...
		auto& entry = _cache.emplace_back();
		entry.Clock = clock;
		_emu->Serialize(entry.SaveState, true, 0);
	}
